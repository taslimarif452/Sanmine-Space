...
function hasExternalWebsite(description = "") {
  ...
  return urls.some((raw) => { try { const host = new URL(raw.startsWith("www.") ? `https://${raw}` : raw).hostname.toLowerCase().replace(/^www\./, ""); return !SOCIAL_HOSTS.has(host); } catch { return false; } });
}
...
function candidateQueries(query: string) { return wantsIndia(query) ? ["Indian creator", "Indian YouTuber", "India creator"] : ["YouTube creator"]; }
async function youtubeJson(url: string) {
  const response = await fetch(url, { cache: "no-store", signal: AbortSignal.timeout(12000) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) { const reason = data?.error?.errors?.[0]?.reason; throw new Error(`${data?.error?.message || `YouTube API request failed (${response.status}).`}${reason ? ` [${reason}]` : ""}`); }
  return data;
}
async function searchChannels(apiKey: string, query: string, region: string, pageToken?: string): Promise<SearchResponse> {
  ...
  if (region) params.set("regionCode", region); if (pageToken) params.set("pageToken", pageToken);
...
async function loadChannels(apiKey: string, channelIds: string[]) {
  ...
  const params = new URLSearchParams({ part: "snippet,statistics", id: channelIds.join(","), key: apiKey });
...
export const youtubeSearchTool: AgentTool = {
  ...
  execute: async (args) => {
    ...
    const collectedIds: string[] = []; const seenIds = new Set<string>(); const usedQueries: string[] = []; let pagesFetched = 0; let nextPageToken: string | undefined;
    for (const query of queries) {
      ...
      for (let page = 0; page < (filterNoWebsite ? 2 : 1); page += 1) {
        ...
        nextPageToken = data.nextPageToken; if (!filterNoWebsite || collectedIds.length >= 50 || !nextPageToken) break;
      }
      if (!filterNoWebsite || collectedIds.length >= 50) break;
    }
    if (!collectedIds.length) return { status: "success", source: "YouTube Data API v3", query: usedQueries.join(" | ") || queries.join(" | "), requested_query: rawQuery, type: "creator", results: [], candidate_count: 0, pages_fetched: pagesFetched, message: "YouTube Data API v3 returned no matching channels for the normalized search query. The API key was accepted by YouTube; this is not a missing-key response." };
    const channels = await loadChannels(apiKey, collectedIds);
    const channelResults = normalizeChannels(channels);
    const filtered = filterNoWebsite ? channelResults.filter((channel) => !channel.website_in_channel_description).slice(0, requestedLimit) : channelResults.slice(0, requestedLimit);
    const response = { status: "success", source: "YouTube Data API v3", query: usedQueries.filter((value, index, all) => all.indexOf(value) === index).join(" | "), requested_query: rawQuery, type: "creator", region_code: region || undefined, candidate_count: channelResults.length, pages_fetched: pagesFetched, results: filtered, website_filter: filterNoWebsite ? "Excluded channels whose public YouTube channel description contains an external website URL. Social-only links were not treated as a website." : undefined, note: filterNoWebsite && filtered.length < requestedLimit ? `Only ${filtered.length} matching channels were found in the inspected YouTube candidate set. YouTube Data API v3 does not expose a definitive has-website field, so website absence is inferred from the public channel description.` : filterNoWebsite ? "Website absence is inferred from the public YouTube channel description; YouTube Data API v3 does not expose a definitive has-website field." : undefined };
    const userId = typeof args.user_id === "string" ? args.user_id.trim() : "";
    if (userId && filtered.length) {
      try {
        await runProductionMigrations();
        await saveResearch(userId, rawQuery, "YouTube Data API v3", filtered.map((channel) => ({ name: String(channel.creator_name || "Creator"), country: channel.country, niche: rawQuery, youtube_channel_id: channel.channel_id, youtube_url: channel.url, subscribers: channel.subscribers, total_views: channel.total_views, description: channel.description, evidence: { website_in_channel_description: channel.website_in_channel_description }, sources: channel.url ? [{ url: channel.url, title: String(channel.creator_name || "YouTube channel"), snippet: String(channel.description || ""), source_type: "youtube" }] : [] })), { region_code: region, website_filter: filterNoWebsite, pages_fetched: pagesFetched });
      } catch (error) {
