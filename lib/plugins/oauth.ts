...
const allowedProviders = new Set<PluginProvider>([
  "github",
  "notion",
  "canva",
  "discord",
...
export function verifyPluginState(state: string) {
  ...
  if (!data.uid || !data.provider || !isPluginProvider(data.provider) || !data.exp || data.exp < Date.now()) {
...
function encryptionKey() {
  return createHmac("sha256", secret("OAUTH_TOKEN_ENCRYPTION_KEY"))
    .update("sanmine-plugin-tokens")
    .digest();
}
...
function config(provider: PluginProvider, origin: string) {
  if (provider === "vercel") {
    const clientId = env("VERCEL_CLIENT_ID") || env("NEXT_PUBLIC_VERCEL_APP_CLIENT_ID");
    const clientSecret = env("VERCEL_CLIENT_SECRET") || env("VERCEL_APP_CLIENT_SECRET");
    if (!clientId || !clientSecret) {
      throw new Error("VERCEL_CLIENT_ID and VERCEL_CLIENT_SECRET are not configured.");
    }
    return {
      clientId,
      clientSecret,
      redirectUri: env("VERCEL_REDIRECT_URI") || `${origin}/api/plugins/vercel/callback`,
    };
  }

  if (provider === "google-workspace") {
    const clientId = secret("GOOGLE_WORKSPACE_CLIENT_ID");
    const clientSecret = secret("GOOGLE_WORKSPACE_CLIENT_SECRET");
    return {
      clientId,
      clientSecret,
      redirectUri: env("GOOGLE_WORKSPACE_REDIRECT_URI") || `${origin}/api/plugins/google-workspace/callback`,
    };
  }
  ...
  const [idName, secretName, redirectName] = values[provider as keyof typeof values];
  const clientId = secret(idName);
  const clientSecret = secret(secretName);
  const redirectUri = env(redirectName) || `${origin}/api/plugins/${provider}/callback`;
...
export function authorizationUrl(
  ...
  if (provider === "canva") {
    ...
    params.set(
      "scope",
      "design:read design:write design:content:read design:content:write asset:read asset:write profile:read",
    );
  }
  if (provider === "discord") params.set("scope", "identify email guilds");
  if (provider === "vercel") params.set("scope", "openid email profile");
  if (provider === "google-workspace") {
...
async function exchange(
  ...
  if (provider === "github") {
    const data = await tokenRequest(
      ...
      new URLSearchParams({ client_id: c.clientId, client_secret: c.clientSecret, code, redirect_uri: c.redirectUri }),
    );
    ...
    if (!r.ok || !profile.id) throw new Error("Unable to read the GitHub account.");
    ...
    const email = Array.isArray(emails)
      ? emails.find((x) => x.primary && x.verified)?.email || emails.find((x) => x.verified)?.email || profile.email
      : profile.email;
    return {
      accessToken,
  ...
  if (provider === "notion") {
    ...
    return {
      ...
      refreshToken: data.refresh_token ? String(data.refresh_token) : null,
      expiresAt: null,
      accountId: String(data.workspace_id || data.bot_id || "notion"),
      email: person?.email ? String(person.email) : null,
      name: owner?.name ? String(owner.name) : data.workspace_name ? String(data.workspace_name) : null,
      metadata: { workspaceId: data.workspace_id, workspaceName: data.workspace_name, botId: data.bot_id },
    };
  }
  ...
  if (provider === "canva") {
    ...
    const data = await tokenRequest(
      ...
      new URLSearchParams({ grant_type: "authorization_code", code, redirect_uri: c.redirectUri, code_verifier: codeVerifier }),
      { Authorization: `Basic ${Buffer.from(`${c.clientId}:${c.clientSecret}`).toString("base64")}` },
    );
    const accessToken = String(data.access_token || "");
    ...
    return {
      ...
      refreshToken: data.refresh_token ? String(data.refresh_token) : null,
      expiresAt: data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : null,
      accountId: String(profile.user_id || profile.id || "canva"),
  ...
  if (provider === "discord") {
    const data = await tokenRequest(
      ...
      new URLSearchParams({ client_id: c.clientId, client_secret: c.clientSecret, grant_type: "authorization_code", code, redirect_uri: c.redirectUri }),
    );
    const accessToken = String(data.access_token || "");
    ...
    const profile = await r.json();
    if (!r.ok || !profile.id) throw new Error("Unable to read the Discord account.");
    return {
      accessToken,
      refreshToken: data.refresh_token ? String(data.refresh_token) : null,
      expiresAt: data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : null,
  ...
  if (provider === "vercel") {
    ...
    const r = await fetch("https://api.vercel.com/login/oauth/userinfo", {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    const profile = await r.json().catch(() => ({}));
    if (!r.ok || !profile.sub) throw new Error("Unable to read the Vercel account.");
    return {
      accessToken,
      refreshToken: data.refresh_token ? String(data.refresh_token) : null,
      expiresAt: data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : Date.now() + 3600000,
  ...
  const profile = await r.json();
  if (!r.ok || !profile.sub) throw new Error("Unable to read the Google account.");
  return {
    accessToken,
    refreshToken: data.refresh_token ? String(data.refresh_token) : null,
    expiresAt: data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : null,
...
export async function createConnection(
  ...
  await sql`INSERT INTO plugin_connections (
    user_id, provider, provider_account_id, email, display_name,
    access_token, refresh_token, expires_at, metadata
  ) VALUES (
    ${uid}, ${provider}, ${token.accountId}, ${token.email}, ${token.name},
    ${encryptPluginToken(token.accessToken)},
    ${token.refreshToken ? encryptPluginToken(token.refreshToken) : null},
    ${token.expiresAt}, ${JSON.stringify(token.metadata || {})}::jsonb
  )`;
}

export async function listConnections(uid: string) {
  await ensureTable();
  return sql`SELECT id, provider, provider_account_id, email, display_name, expires_at, metadata, created_at, updated_at
    FROM plugin_connections WHERE user_id = ${uid} ORDER BY updated_at DESC`;
}
...
async function refresh(provider: PluginProvider, row: Record<string, unknown>) {
  ...
  if (provider === "github") {
    ...
    data = await tokenRequest("https://github.com/login/oauth/access_token", body);
  } else if (provider === "notion") {
    data = await tokenRequest("https://api.notion.com/v1/oauth/token", body, {
      Authorization: `Basic ${Buffer.from(`${c.clientId}:${c.clientSecret}`).toString("base64")}`,
    });
  } else if (provider === "canva") {
    data = await tokenRequest("https://api.canva.com/rest/v1/oauth/token", body, {
      Authorization: `Basic ${Buffer.from(`${c.clientId}:${c.clientSecret}`).toString("base64")}`,
    });
  } else if (provider === "discord") {
    body.set("client_id", c.clientId);
    body.set("client_secret", c.clientSecret);
    data = await tokenRequest("https://discord.com/api/v10/oauth2/token", body);
  } else if (provider === "vercel") {
    body.set("client_id", c.clientId);
    body.set("client_secret", c.clientSecret);
    data = await tokenRequest("https://api.vercel.com/login/oauth/token", body);
  } else {
    body.set("client_id", c.clientId);
    body.set("client_secret", c.clientSecret);
    data = await tokenRequest("https://oauth2.googleapis.com/token", body);
  }
  ...
  await sql`UPDATE plugin_connections
    SET access_token = ${encryptPluginToken(accessToken)},
        refresh_token = ${encryptPluginToken(nextRefresh)},
...
export async function getAccessToken(uid: string, provider: PluginProvider) {
  ...
  if (expiresAt && expiresAt < Date.now() + 60000 && row.refresh_token) return refresh(provider, row);
...
export async function disconnectConnection(uid: string, id: string) {
  ...
  try {
    if (provider === "github") {
      await fetch(`https://api.github.com/applications/${encodeURIComponent(c.clientId)}/grant`, {
        method: "DELETE",
        headers: {
          Authorization: `Basic ${Buffer.from(`${c.clientId}:${c.clientSecret}`).toString("base64")}`,
