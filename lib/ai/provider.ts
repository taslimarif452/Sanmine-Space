...
export type ProviderResponse = { text: string; toolCalls: ToolCall[]; raw?: unknown; usage?: ProviderUsage; provider?: AIProviderName; model?: string };
...
export function getProvider(task: ProviderTask = "balanced"): AIProvider {
  const selected = (process.env.AI_PROVIDER || "gemini").trim().toLowerCase();
  return selected === "openrouter"
    ? new ResilientProvider(new OpenRouterProvider(task), new GeminiProvider(task))
    : new ResilientProvider(new GeminiProvider(task), new OpenRouterProvider(task));
}
...
async function readProviderError(response: Response, provider: string) {
  ...
  try { const parsed = JSON.parse(raw); detail = parsed?.error?.message || parsed?.message || raw || detail; }
...
function geminiModel(task: ProviderTask) {
  if (task === "fast") return process.env.GEMINI_FAST_MODEL?.trim() || process.env.GEMINI_MODEL?.trim() || "gemini-3.5-flash";
  if (task === "deep") return process.env.GEMINI_DEEP_MODEL?.trim() || process.env.GEMINI_MODEL?.trim() || "gemini-3.5-flash";
  return process.env.GEMINI_BALANCED_MODEL?.trim() || process.env.GEMINI_MODEL?.trim() || "gemini-3.5-flash";
}

function openRouterModel(task: ProviderTask) {
  if (task === "fast") return process.env.OPENROUTER_FAST_MODEL?.trim() || process.env.OPENROUTER_MODEL?.trim() || "openrouter/free";
  if (task === "deep") return process.env.OPENROUTER_DEEP_MODEL?.trim() || process.env.OPENROUTER_MODEL?.trim() || "openrouter/free";
  return process.env.OPENROUTER_BALANCED_MODEL?.trim() || process.env.OPENROUTER_MODEL?.trim() || "openrouter/free";
}

class GeminiProvider implements AIProvider {
  constructor(private readonly task: ProviderTask = "balanced") {}
  async chat(messages: ChatMessage[], tools: ToolDefinition[] = []): Promise<ProviderResponse> {
    const key = process.env.GEMINI_API_KEY?.trim();
    if (!key) throw new Error("GEMINI_API_KEY is not configured in Vercel.");
    const model = geminiModel(this.task);
    ...
    return { ...parseGeminiResponse(data), provider: "gemini", model, usage: normalizeGeminiUsage(data) };
  }
  ...
  async chatStream(messages: ChatMessage[], tools: ToolDefinition[] = [], onText?: (delta: string) => void): Promise<ProviderResponse> {
    ...
    if (!response.ok) throw new Error(await readProviderError(response, `Gemini ${model}`));
    const parts: Array<{ text?: string; functionCall?: { name?: string; args?: Record<string, unknown> }; usageMetadata?: ProviderUsage }> = [];
    let usage: ProviderUsage | undefined;
    await readSse(response, (data) => {
      if (data.usageMetadata) usage = normalizeGeminiUsage(data);
      for (const part of data.candidates?.[0]?.content?.parts || []) { parts.push(part); if (part.text) onText?.(part.text); }
    });
    ...
    return { text, toolCalls, provider: "gemini", model, usage };
  }
}
...
function normalizeGeminiUsage(data: any): ProviderUsage | undefined {
  const u = data?.usageMetadata;
  if (!u) return undefined;
  const inputTokens = Number(u.promptTokenCount || 0);
  const outputTokens = Number(u.candidatesTokenCount || 0);
  return { inputTokens, outputTokens, totalTokens: Number(u.totalTokenCount || inputTokens + outputTokens) };
}
...
class OpenRouterProvider implements AIProvider {
  ...
  async chat(messages: ChatMessage[], tools: ToolDefinition[] = []): Promise<ProviderResponse> {
    ...
    const model = openRouterModel(this.task);
    ...
    const data = await response.json();
    return { ...parseOpenRouterResponse(data), provider: "openrouter", model, usage: normalizeOpenRouterUsage(data) };
  }
  ...
  async chatStream(messages: ChatMessage[], tools: ToolDefinition[] = [], onText?: (delta: string) => void): Promise<ProviderResponse> {
    ...
    await readSse(response, (data) => {
      if (data.usage) usage = normalizeOpenRouterUsage(data);
      const delta = data.choices?.[0]?.delta;
      if (delta?.content) { text += String(delta.content); onText?.(String(delta.content)); }
    ...
    return { text, toolCalls, provider: "openrouter", model, usage };
  }
}
...
function normalizeOpenRouterUsage(data: any): ProviderUsage | undefined {
  const u = data?.usage;
  if (!u) return undefined;
  const inputTokens = Number(u.prompt_tokens || 0);
  const outputTokens = Number(u.completion_tokens || 0);
  return { inputTokens, outputTokens, totalTokens: Number(u.total_tokens || inputTokens + outputTokens) };
}
