...
export default function Home() {
  ...
  return <main className="flex h-screen min-h-0 overflow-hidden bg-[var(--bg)]">
    ...
    <section className="relative flex h-screen min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      {messages.length === 0 ? <div className="flex min-h-0 flex-1 flex-col items-center overflow-hidden px-4 pb-8 pt-[15vh]"><div className="w-full max-w-[760px]"><div className="mb-8 text-center"><div className="mx-auto mb-5 grid h-11 w-11 place-items-center overflow-hidden rounded-2xl"><img src={BRAND_LOGO} alt="Sanmine Space" className="h-full w-full object-cover" /></div><h1 className="font-serif text-4xl tracking-[-0.035em] text-[#282721] md:text-[46px]">How can I help?</h1><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[var(--muted)]">Research leads, build outreach campaigns, and get work done from one simple conversation.</p></div><Composer message={message} setMessage={setMessage} submit={submit} loading={loading} /><Suggestions setMessage={setMessage} />{error && <ErrorMessage message={error} />}</div></div> : <div className="flex min-h-0 flex-1 flex-col overflow-hidden"><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-10"><div className="mx-auto w-full max-w-[820px]"><div className="space-y-8">{messages.map((item, index) => <div key={`${item.role}-${index}`} className={item.role === "user" ? "flex justify-end" : "flex justify-start"}>{item.role === "user" ? <div className="max-w-[75%] rounded-2xl bg-[#ebe9e3] px-4 py-3 text-[15px] leading-6 text-[#282721]">{item.content}</div> : <div className="w-full max-w-[85%] text-[15px] leading-7 text-[#37352f]"><div className="flex gap-3"><div className="mt-1 grid h-7 w-7 shrink-0 place-items-center overflow-hidden rounded-lg"><img src={BRAND_LOGO} alt="" className="h-full w-full object-cover" /></div><div className="whitespace-pre-wrap">{item.content}</div></div>{item.sources?.length ? <SourceCards sources={item.sources} /> : null}</div>}</div>)}{loading && <div className="flex items-center gap-3 text-sm text-[var(--muted)]"><div className="grid h-7 w-7 shrink-0 place-items-center overflow-hidden rounded-lg"><img src={BRAND_LOGO} alt="" className="h-full w-full object-cover" /></div><span className="flex items-center gap-2">Researching <Loader2 size={14} className="animate-spin" /></span></div>}{error && <ErrorMessage message={error} />}</div></div></div><div className="mx-auto w-full max-w-[820px] shrink-0 px-4 pb-6 pt-2"><Composer message={message} setMessage={setMessage} submit={submit} loading={loading} /></div></div>}
    </section>
  </main>;
}
...
function ErrorMessage({ message }: { message: string }) { return <div className="mt-3 rounded-xl border border-[#e7c9bd] bg-[#fff8f5] px-4 py-3 text-sm text-[#8c4b36]">{message}</div>; }
