...
function Composer({ message, setMessage, submit, loading }: { message: string; setMessage: (value: string) => void; submit: () => void; loading: boolean }) { return <div className="rounded-[22px] border border-[#dcd9d1] bg-[var(--panel)] p-2 shadow-[0_8px_35px_rgba(30,27,20,0.06)] focus-within:border-[#c8c4ba]"><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); submit(); } }} rows={3} placeholder="Ask Sanmine Space to do something..." className="min-h-[92px] w-full resize-none border-0 bg-transparent px-4 py-3 text-[15px] leading-6 outline-none placeholder:text-[#aaa69d]" disabled={loading} /><div className="flex items-center justify-between px-2 pb-1"><span className="px-2 text-[11px] text-[#aaa69d]">Enter to send · Shift + Enter for new line</span><button onClick={submit} disabled={!message.trim() || loading} className="grid h-9 w-9 place-items-center rounded-xl bg-[#24231f] text-white disabled:opacity-25" aria-label="Send">{loading ? <Loader2 size={17} className="animate-spin" /> : <ArrowUp size={17} />}</button></div></div>; }
function Suggestions({ setMessage }: { setMessage: (value: string) => void }) { return <div className="mt-5 flex flex-wrap justify-center gap-2">{examples.map((example) => <button key={example} onClick={() => setMessage(example)} className="rounded-full border border-[var(--line)] bg-white/40 px-3.5 py-2 text-xs text-[#68655e] hover:bg-white">{example}</button>)}</div>; }
