...
import { encryptToken, exchangeMicrosoftCode, verifyOAuthState } from "@/lib/email/oauth";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!code || !state) return redirect("microsoft_error");
    const cookieStore = await cookies();
    const savedState = cookieStore.get("sanmine_microsoft_oauth_state")?.value;
    ...
    await sql`INSERT INTO email_connections (user_id, provider, email, display_name, access_token, refresh_token, expires_at, updated_at)
      VALUES (${stateData.uid}, 'microsoft', ${token.email}, ${token.name}, ${encryptToken(token.accessToken)}, ${token.refreshToken ? encryptToken(token.refreshToken) : null}, ${token.expiresAt}, NOW())
    ...
    const response = redirect("microsoft_connected");
    response.cookies.delete("sanmine_microsoft_oauth_state");
    return response;
  } catch (error) {
    console.error("Microsoft OAuth callback failed", error);
    const response = redirect("microsoft_error");
    response.cookies.delete("sanmine_microsoft_oauth_state");
    return response;
  }
}
