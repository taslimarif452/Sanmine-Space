...
export async function GET(request: Request) {
  try {
    ...
    await enforceRateLimit(`campaigns:get:${user.uid}`, 60, 60_000);
    ...
    return NextResponse.json(errorResponse(error, "Unable to load campaigns."), { status: errorStatus(error) });
  }
}
...
export async function POST(request: Request) {
  try {
    const user = await getRequestUser(request);
    await runProductionMigrations();
    await enforceRateLimit(`campaigns:create:${user.uid}`, 10, 60 * 60_000);
    const parsed = CreateCampaignSchema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) throw new AppError("VALIDATION_ERROR", parsed.error.issues[0]?.message || "Invalid campaign request.", 400);
    const { name, connectionId, subject, body, recipients, intervalMinutes, startAt } = parsed.data;
    const start = startAt ? new Date(startAt) : new Date();
    const uniqueRecipients = [...new Set(recipients)];
    if (!uniqueRecipients.length) throw new AppError("VALIDATION_ERROR", "At least one unique recipient is required.", 400);

    const connections = await sql`SELECT id, provider FROM email_connections WHERE id=${connectionId} AND user_id=${user.uid} LIMIT 1`;
    if (!connections[0]) throw new AppError("NOT_FOUND", "Email connection not found.", 404);
    if (connections[0].provider !== "google") throw new AppError("VALIDATION_ERROR", "Only Gmail campaigns are enabled right now.", 400);

    const rows = await sql`
      WITH campaign AS (
        INSERT INTO campaigns (user_id, connection_id, name, subject, body, status, start_at, interval_minutes)
        VALUES (${user.uid}, ${connectionId}, ${name}, ${subject}, ${body}, 'draft', ${start.toISOString()}, ${intervalMinutes})
        RETURNING *
      ), approvals AS (
    ...
    return NextResponse.json(errorResponse(error, "Unable to create campaign."), { status: errorStatus(error) });
  }
}
