    ...
    <main className="flex h-screen min-h-0 overflow-hidden bg-[var(--bg)] text-[var(--text)]">
      ...
      <section className="min-w-0 flex-1 overflow-y-auto">
        <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between bg-[var(--bg)] px-5 sm:px-7 md:hidden">
          <button type="button" onClick={() => window.history.back()} className="grid h-12 w-12 place-items-center rounded-full bg-white text-[#171614] shadow-[0_1px_5px_rgba(0,0,0,.06)]" aria-label="Go back">
            <ArrowLeft size={25} strokeWidth={2.2} />
          </button>

          <div className="relative">
            <button type="button" onClick={() => setCategoryOpen((open) => !open)} className="flex items-center gap-1 text-[31px] font-semibold tracking-[-.04em] text-[#171614]" aria-haspopup="menu" aria-expanded={categoryOpen}>
              Plugins
          ...
          <button type="button" className="grid h-12 w-12 place-items-center rounded-full bg-white text-[#171614] shadow-[0_1px_5px_rgba(0,0,0,.06)]" aria-label="Plugin settings">
        ...
        <div className="mx-auto w-full max-w-[1120px] px-5 pb-12 pt-5 sm:px-8 sm:pt-8 md:px-10 md:pb-16 md:pt-12">
          <div className="hidden items-center justify-between md:flex">
            <div className="relative">
              <button type="button" onClick={() => setCategoryOpen((open) => !open)} className="flex items-center gap-1 text-3xl font-semibold tracking-[-.035em] text-[#282721]" aria-haspopup="menu" aria-expanded={categoryOpen}>
                Plugins
                <ChevronDown size={18} strokeWidth={2.3} className="mt-1" />
              </button>
              {categoryOpen && <CategoryMenu cat={cat} setCat={setCat} close={() => setCategoryOpen(false)} />}
            </div>
            <button type="button" className="grid h-11 w-11 place-items-center rounded-full bg-white text-[#38352f] shadow-[0_1px_5px_rgba(0,0,0,.05)]" aria-label="Plugin settings">
              <Settings size={21} />
            </button>
          </div>
          ...
          <div className="mt-7 md:mt-6">
            <label className="flex h-[56px] w-full items-center gap-3 rounded-[28px] border border-[#dedbd5] bg-white px-5 shadow-[0_1px_3px_rgba(0,0,0,.025)] md:h-12 md:max-w-[420px] md:rounded-2xl md:px-4">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search plugins" className="min-w-0 flex-1 bg-transparent text-[17px] text-[#302e29] outline-none placeholder:text-[#999792] md:text-sm" />
              {q && <button type="button" onClick={() => setQ("")} className="text-[#96928b]" aria-label="Clear search"><X size={17} /></button>}
            </label>
          </div>
          ...
          <section className="mt-12 md:mt-10">
            ...
            <div className="grid grid-cols-1 gap-0 md:grid-cols-2 md:gap-x-12 lg:gap-x-20">
              {filtered.map((plugin) => (
                <article key={plugin.id} className="group flex min-h-[84px] items-center gap-4 py-4 md:min-h-[76px] md:py-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-[16px] font-medium text-[#24231f] md:text-[13px]">{plugin.name}</h3>
  ...
  return (
    <div className="absolute left-1/2 top-full z-40 mt-3 w-48 -translate-x-1/2 rounded-2xl border border-[#e4e1db] bg-white p-1.5 shadow-[0_10px_30px_rgba(0,0,0,.08)]" role="menu">
      {categories.map((category) => (
        <button key={category} type="button" onClick={() => { setCat(category); close(); }} className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm ${cat === category ? "bg-[#f2f0eb] font-medium text-[#282721]" : "text-[#6d6961] hover:bg-[#f7f6f3]"}`} role="menuitem">
          <span>{category}</span>
