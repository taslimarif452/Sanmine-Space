...
export function CookieConsent(){
  ...
  return <div className="fixed inset-x-0 bottom-0 z-[100] w-full p-0">
    <section role="dialog" aria-label="Cookie settings" className="min-h-[330px] w-full rounded-t-[14px] border border-b-0 border-[#383838] bg-[#171717] px-4 py-7 text-[#f5f5f5] shadow-[0_20px_70px_rgba(45,42,35,.28)] sm:min-h-[330px] sm:px-6 sm:py-7">
      ...
      <label className="mt-3 flex cursor-pointer items-start gap-2.5 text-[12.5px] leading-[1.4] text-[#d7d7d1]">
      ...
      <button type="button" onClick={()=>setCustomizing(v=>!v)} className="mt-4 w-full rounded-[9px] bg-[#303030] px-4 py-3 text-[14px] font-medium transition hover:bg-[#393939]">{customizing?"Close cookie preferences":"Customize Cookie Settings"}</button>
      {customizing&&<div className="mt-3 rounded-[9px] border border-[#3a3a3a] bg-[#202020] p-3 text-sm">
        ...
        <button type="button" onClick={()=>save("customized")} disabled={!legalAccepted} className="mt-3 w-full rounded-[9px] bg-white px-4 py-2 text-sm font-medium text-[#171717] transition hover:bg-[#eee] disabled:cursor-not-allowed disabled:opacity-50">Save preferences</button>
      </div>}
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <button type="button" onClick={()=>save("rejected")} className="rounded-[9px] bg-[#303030] px-4 py-3 text-[14px] font-medium transition hover:bg-[#393939]">Reject All Cookies</button>
        <button type="button" onClick={()=>save("accepted")} disabled={!legalAccepted} className="rounded-[9px] bg-white px-4 py-3 text-[14px] font-medium text-[#171717] transition hover:bg-[#eee] disabled:cursor-not-allowed disabled:opacity-50">Accept All Cookies</button>
      </div>
    </section>
  </div>;
}
