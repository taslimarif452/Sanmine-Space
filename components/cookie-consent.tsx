...
export function CookieConsent(){
  ...
  return <div className="fixed inset-x-0 bottom-0 z-[100] p-3 sm:p-4">
    <section role="dialog" aria-label="Cookie settings" className="mx-auto max-w-[560px] rounded-[16px] border border-[#383838] bg-[#171717] p-4 text-[#f5f5f5] shadow-[0_20px_70px_rgba(0,0,0,.28)] sm:p-5">
      ...
      <label className="mt-4 flex cursor-pointer items-start gap-2.5 text-[13px] leading-5 text-[#d7d7d1]">
        <input type="checkbox" checked={legalAccepted} onChange={e=>setLegalAccepted(e.target.checked)} className="mt-1 h-4 w-4 shrink-0 accent-white" />
      ...
      {customizing&&<div className="mt-3 rounded-[10px] border border-[#3a3a3a] bg-[#202020] p-3.5 text-sm">
        ...
        <button type="button" onClick={()=>save("customized")} disabled={!legalAccepted} className="mt-3 w-full rounded-[10px] bg-white px-4 py-2.5 text-sm font-medium text-[#171717] transition hover:bg-[#eee] disabled:cursor-not-allowed disabled:opacity-50">Save preferences</button>
      </div>}
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        <button type="button" onClick={()=>save("accepted")} disabled={!legalAccepted} className="rounded-[10px] bg-white px-4 py-2.5 text-[15px] font-medium text-[#171717] transition hover:bg-[#eee] disabled:cursor-not-allowed disabled:opacity-50">Accept All Cookies</button>
      </div>
    </section>
  </div>;
}
