...
export function WorkspaceSidebar({user}:{user:User}){
  ...
  useEffect(()=>{
    ...
    window.addEventListener("sanmine:open-sidebar",open);
    return()=>window.removeEventListener("sanmine:open-sidebar",open);
  },[]);
  ...
  const remove=async(id:string)=>{
    ...
    try{
      const token=await user.getIdToken();
      const r=await fetch(`/api/chats/${id}/manage`,{method:"DELETE",headers:{Authorization:`Bearer ${token}`}});
      if(!r.ok)throw new Error("Unable to delete chat.");
      setChats(xs=>xs.filter(c=>c.id!==id));
      if(pathname===`/chat/${id}`)fresh();
    }catch{}
    setMenu(null);
  };
  ...
  return <>
    ...
    <aside data-sanmine-workspace-sidebar className={`${mobileOpen?"flex":"hidden"} fixed inset-y-0 left-0 z-40 w-[78vw] max-w-[310px] shrink-0 flex-col overflow-y-auto overscroll-contain border-r border-[var(--line)] bg-[#f2f1ed] p-3 shadow-xl md:sticky md:top-0 md:flex md:h-screen md:max-h-screen md:shadow-none ${side?"md:w-[270px]":"md:w-[72px]"}`}>
      <div className={`sticky top-0 z-20 flex shrink-0 items-center bg-[#f2f1ed] pb-5 pt-1 ${side?"justify-between px-2":"justify-center"}`}>
