...
export function EmailConnections({ user }: { user: User }) {
  ...
  const load = async () => {
    try {
      ...
      setConnections((Array.isArray(data.connections) ? data.connections : []).filter((x: Connection) => x.provider === "google"));
  ...
  const connect = async () => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to start Gmail connection."); setBusy(null); }
  };
  ...
  return <div className="mt-2">
    ...
    <div className="space-y-1">
      ...
      <button onClick={() => void connect()} disabled={busy !== null} className="flex w-full items-center gap-3 rounded-xl border border-[#e8e4dc] bg-white/60 px-3 py-3 text-sm hover:bg-white disabled:opacity-40"><span className="grid h-8 w-8 place-items-center rounded-full bg-white"><GoogleMark/></span><span className="flex-1 text-left">{busy === "google" ? "Connecting…" : "Connect Gmail"}</span><PlugZap size={14}/></button>
    </div>
    <p className="mt-3 text-xs leading-5 text-[#969188]">Outlook is disabled for now. Gmail is the active sending provider for approvals and campaigns.</p>
